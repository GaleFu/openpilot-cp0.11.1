...
async def get_schema(request: 'web.Request'):
  # Keep a useful default for health checks and older clients that omitted the
  # query parameter.  Missing `services` previously raised KeyError and aiohttp
  # returned HTTP 500 even though WebRTC itself was healthy.
  services = request.query.get("services", "carState,modelV2,longitudinalPlan,driverAssistance").split(",")
  services = [s for s in services if s]
  invalid = [s for s in services if s not in log.Event.schema.fields or s.endswith("DEPRECATED")]
  if invalid:
    raise web.HTTPBadRequest(text=f"Invalid service name: {','.join(invalid)}")
