  ...
  async def recv(self):
    ...
    while True:
      try:
        ...
        # Use a local, continuous RTP clock.  Camera frameId can start at a
        # large value or jump after camerad/encoderd restarts; mapping it
        # directly to PTS creates a long apparent gap and browsers may keep
        # the track in "waiting for first frame" forever.  Source frame IDs
        # are still retained for overlay sync, while transport timestamps are
        # always paced at the camera rate.
        packet_pts = self._pts
        # Some camera/encoder restarts reset frameId to zero.  Keep RTP PTS
        # strictly increasing so aiortc does not discard the recovered video
        # track (which otherwise appears as a blank WebRTC view).
        if packet_pts <= self._last_packet_pts:
          packet_pts = self._last_packet_pts + 1
        packet.pts = packet_pts

        self.log_debug("track sending frame %s source frame %s", packet_pts, frame_id)
        self._last_packet_pts = packet_pts
