...
class GuiApplication:
  def __init__(self, width: int | None = None, height: int | None = None):
    ...
    # Do not rotate a manual recording into many short MP4 fragments.
    self._record_max_sec = 3600
    self._record_t0 = 0.0
    # Capture every UI frame. This keeps the number of raw frames identical to
    # the encoder input rate, preventing accelerated playback when the UI
    # render cadence varies under load.
    self._record_every_n = 3
    self._record_frame_idx = 0
    self._record_next_capture_time = 0.0
  ...
  def start_recording(self):
    ...
    # The camera/HUD stream is 20 FPS on C3X. Match recording cadence to that
    # rate even when the UI renderer itself is running at 60 FPS.
    # Raylib may run the render loop at 60 Hz even when the camera stream is
    # 20 Hz. Sample exactly one frame out of three and encode at 20 FPS.
    self._record_every_n = 1
    self._record_next_capture_time = time.monotonic()
  ...
  def _init_ffmpeg(self, out_path: Path):
    ...
    ffmpeg_args = [
      ...
      "-fps_mode", "cfr",
      # Keep one output frame per raw input frame.  A second ``-r`` filter can
      # duplicate/drop frames and alter playback speed when the UI cadence is
      # irregular, which was the source of the fast-forward recordings.
      "-c:v", "libx264",
      "-preset", preset,
    ...
    # Never discard captured frames: dropping queue items while ffmpeg keeps a
    # fixed 20 FPS clock shortens the resulting video and appears as fast
    # forward playback. A larger queue absorbs brief encoder stalls.
    # Raw RGBA frames are several MB each; a large queue exhausts C3X RAM.
    self._ffmpeg_queue = queue.Queue(maxsize=3)
  ...
  def init_window(self, title: str, fps: int = _DEFAULT_FPS, font_weights: Iterable[FontWeight] | None = None):
    with self._startup_profile_context():
      ...
      if RECORD:
        # Camera/on-road video cadence is 20 FPS on C3X. Use the same clock
        # as manual recording so the always-on path cannot create fast video.
        record_fps = 20.0
        ffmpeg_args = [
          'ffmpeg',
          ...
          '-r', str(record_fps),    # Input frame rate
      ...
      # Keep the raw-frame sampling rate aligned with the 20 FPS camera stream
      # for the always-on RECORD path as well as manual recordings.
      self._record_every_n = 1
      self._record_next_capture_time = time.monotonic()
      self._set_styles()
    ...
    try:
      ...
      while not (self._window_close_requested or rl.window_should_close()):
        ...
        if RECORD or self._record_enabled:
          ...
          capture_due = time.monotonic() >= self._record_next_capture_time
          if capture_due:
            capture_now = time.monotonic()
            # If rendering stalled, enqueue one copy per elapsed 50 ms slot so
            # the MP4 timeline keeps real duration instead of playing fast.
            due_count = max(1, min(8, int((capture_now - self._record_next_capture_time) / 0.05) + 1))
            self._record_next_capture_time += due_count * 0.05
            image = rl.load_image_from_texture(self._render_texture.texture)
            data_size = image.width * image.height * 4
            data = bytes(rl.ffi.buffer(image.data, data_size))
            # Keep only one raw frame per capture tick. Never duplicate large
            # buffers when the UI stalls; bounded memory is more important than
            # synthetic repeats that would increase RAM pressure.
            try:
          ...
          # A manual recording remains in one file. It is stopped explicitly
          # by the HUD button (or when the user exits recording mode), avoiding
          # the previous automatic 60-second stop/start fragmentation.
