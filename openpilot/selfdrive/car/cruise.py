...
class VCruiseCarrot:
  def __init__(self, CP):
    ...
    self.is_changan = CP.brand == "changan"
  ...
  def _prepare_buttons(self, CS, v_cruise_kph):
    ...
    for b in buttonEvents:
      ...
      # A fresh Z6 RES+/SET- event is itself a new physical press edge.  Let
      # it restart stale hold tracking even if the preceding release event
      # was missed, instead of permanently rejecting all later presses.
      changan_speed_press = self.is_changan and b.pressed and bt in [
        ButtonType.accelCruise, ButtonType.decelCruise
      ]
      if changan_speed_press or (b.pressed and self.button_cnt == 0 and bt in [
        ButtonType.accelCruise, ButtonType.decelCruise,
        ...
        # Z6 reliably supplies the press edge, while the release edge can be
        # lost. Feed the press into the original 0111 speed policy immediately.
        if self.is_changan and bt in [ButtonType.accelCruise, ButtonType.decelCruise]:
          # Both Z6 rocker directions are direct fine adjustments.  Using the
          # mode-dependent SET- unit here can turn one short press into a 10
          # km/h jump while RES+ moves only 1 km/h.
          unit = SPEED_UP_UNIT
          unit = unit if is_metric else unit * CV.MPH_TO_KPH
        ...
        if self.is_changan and bt in [ButtonType.accelCruise, ButtonType.decelCruise]:
          # The original 0111 policy already consumed this press.
          pass
  ...
  def _update_cruise_buttons(self, CS, CC, v_cruise_kph):
    ...
    if not long_pressed:
      if button_type == ButtonType.accelCruise:
        self._lat_enabled = True
        self._pause_auto_speed_up = False
        if self.is_changan and CC.enabled and not CS.cruiseState.standstill:
          # Z6 consumes RES+ on the press edge.  The generic 0111 mode 1/2/3
          # branches derive another target from navigation/current speed and
          # can discard button_kph entirely.  While actively driving, keep the
          # driver's explicit button result authoritative for this cycle.
        ...
        if self.is_changan and CC.enabled and not CS.cruiseState.standstill:
          # As with RES+, do not let CruiseButtonMode replace the SET- result
          # with vehicle speed or carrot-cruise state.  This is the value that
          # becomes CS.vCruiseCluster and therefore the 0111 planner target.
          v_cruise_kph = button_kph
          self.carrot_cruise_active = False
        elif self._soft_hold_active > 0:
