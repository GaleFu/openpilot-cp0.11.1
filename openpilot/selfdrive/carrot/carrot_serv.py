...
# Carrot/CP navigation reports the upcoming junction about 30 m too far away
# on this vehicle.  Correct the service distance itself so auto-turn, speed
# reduction and the HUD all use the same physical distance.  Keep the raw
# nTBTDist values untouched for diagnostics and protocol compatibility.
TURN_DISTANCE_CORRECTION_M = 30


def _correct_turn_distance(distance_m: int | float) -> int:
  """Return a non-negative maneuver distance corrected for the Z6 offset."""
  distance = int(distance_m)
  return max(0, distance - TURN_DISTANCE_CORRECTION_M) if distance > 0 else 0
...
class CarrotServ:
  ...
  def _update_tbt(self):
    ...
    if self.nTBTDist > 0 and self.xTurnInfo > 0:
      self.xDistToTurn = _correct_turn_distance(self.nTBTDist)
    if self.nTBTDistNext > 0 and self.xTurnInfoNext > 0:
      # The next maneuver is cumulative from the current position, so apply
      # one correction to the same cumulative distance rather than twice.
      self.xDistToTurnNext = _correct_turn_distance(self.nTBTDistNext + self.nTBTDist)
  ...
  def update_nav_instruction(self, sm):
    if sm.alive['navInstruction'] and sm.valid['navInstruction']:
      ...
      self.xDistToTurn = _correct_turn_distance(msg_nav.maneuverDistance)
  ...
  def update_navi(self, remote_ip, sm, pm, vturn_speed, coords, distances, route_speed, gps_service):
    ...
    if self.active_carrot > 1 and self.active_kisa_count <= 0:
      ...
      # Publish the same corrected distance used by carrotMan/auto-turn so
      # external clients cannot reintroduce the 30 m late-turn offset.
      instruction.maneuverDistance = float(self.xDistToTurn)
