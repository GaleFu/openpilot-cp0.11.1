...
function getCarrotVisionRealtimeBlockReason() {
  ...
  // Web viewers must be able to inspect the camera while the vehicle is
  // parked/offroad.  The previous parked guard prevented /stream negotiation
  // entirely, leaving the page stuck on “Preparing”.  Driving controls remain
  // governed by the device; this gate only controls remote video display.
  return "";
}
...
async function syncCarrotVisionAvailability() {
  try {
    ...
    // Camera viewing is independent of DisableDM.  DisableDM only controls
    // the on-device HUD mode; requiring it here made normal C3X HUD units
    // appear unavailable and never start WebRTC.
    const available = !clusterHudActive;
...
function rtcInitAuto() {
  ...
  syncCarrotVisionStartOverlay();
  syncCarrotVisionAvailability().then((available) => {
    // Start the remote camera automatically when the Driving page is opened.
    // The explicit button remains available as a fallback for older layouts.
    if (available && isCarrotPageActive() && !isCarrotVisionActive()) {
      window.CarrotVisionStart?.();
    }
  }).catch(() => {});
  rtcBindVideoEvents();
}
...
function syncCarrotRealtimeLifecycle(forceFetch = false, options = {}) {
  ...
  // Keep the realtime transport alive for the visible Driving page while
  // Vision is active.  HUD/overlay consumers can briefly toggle during a
  // layout refresh; treating those pulses as a full idle transition tears
  // down the WebRTC session and causes repeated /stream retries with no frame.
  // Do not force the high-rate HUD/data transport merely because the page is
  // visible.  That keeps the device UI smooth; the Vision lifecycle below
  // owns the WebRTC session independently and can keep video alive without
  // continuously running every HUD data subscriber.
