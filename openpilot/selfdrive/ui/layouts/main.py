...
class MainLayout(Widget):
  def __init__(self):
    ...
    self._cluster_hud_next_refresh = 0.0
  ...
  def _handle_carrot_record_cmd(self, sm) -> bool:
    ...
    # A HUD button toggles the recorder immediately and writes ScreenRecord
    # asynchronously.  Treat an already-running local recorder as authoritative
    # for this frame so the periodic parameter sync cannot stop it again before
    # the nonblocking write is acknowledged.
    local_recording = gui_app.is_recording()
    # ``TimedSnapshotCache`` is the sole parameter source here.  A synchronous
    # Params read on every render tick can block on storage/IPC and freeze the
    # ordinary HUD, especially while the video encoder is active.
    if local_recording and not screen_record:
      screen_record = True
      self._screen_record_param.store_pending(True, time.monotonic())
    # Do not call the recorder start/stop API on every UI frame.  Starting the
    # encoder is not a cheap idempotent operation on C3X; repeatedly issuing
    # the same command can contend with the camera/UI thread and make the
    # on-road view visibly stutter.  Only transition when the state changes.
    desired_recording = bool(screen_record and ui_state.started)
    if desired_recording != local_recording:
  ...
  def _render(self, _):
    ...
    if ui_state.started:
      # Cluster HUD connectivity changes infrequently.  Reading Params here
      # synchronously every render tick can block the on-road UI thread.
      now = time.monotonic()
      if now >= self._cluster_hud_next_refresh:
        try:
          self._cluster_hud_connected = ui_state.params.get_bool("ClusterHudConnected")
        except Exception:
          pass
        self._cluster_hud_next_refresh = now + 1.0
      self._layouts[MainState.ONROAD].set_cluster_hud_connected(
        self._cluster_hud_connected,
