...
def read_border_params(params, params_memory) -> BorderParamSnapshot:
  ...
  # Display-only branding for the Changan Z6.  Identification, fingerprint
  # matching, and control still use the original CarParams/CarName values.
  custom_z6_title = car_name.upper() in (
    "CHANGAN_Z6", "CHANGAN Z6", "CHANGAN_Z6_IDD", "CHANGAN Z6 IDD",
  )
  top_left_op_long = (
    "长安欧尚 与美好生活同行"
    if custom_z6_title and not hyundai_camera_scc
    else (top_left if hyundai_camera_scc else car_name + " - OP Long")
  )
  if nnff_model_name:
    top_left += ",NNFF"
    if not custom_z6_title:
