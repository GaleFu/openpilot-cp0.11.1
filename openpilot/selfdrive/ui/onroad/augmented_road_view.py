...
class AugmentedRoadView(CameraView):
  def __init__(self, stream_type: VisionStreamType = VisionStreamType.VISION_STREAM_ROAD):
    ...
    self._ui_debug_next_publish = 0.0
  ...
  def _render(self, rect):
    ...
    # uiDebug is diagnostic data, not part of the video frame path. Publishing
    # a capnp message every render tick adds serialization and IPC work to the
    # same thread that draws the HUD. Keep telemetry at 5 Hz instead.
    now = time.monotonic()
    if now >= self._ui_debug_next_publish:
      msg = messaging.new_message('uiDebug', valid=True)
      ud = msg.uiDebug
      ud.drawTimeMillis = (now - start_draw) * 1000
      ...
      self._ui_debug_next_publish = now + 0.2
