...
# Chinese weekday labels in Sunday-first display order.
WEEKDAYS_ZH = ("日", "一", "二", "三", "四", "五", "六")

# Labels used by the compact lower-left status panel.  The underlying
# navigation/deceleration code intentionally keeps short protocol names, but
# the on-road HUD should present them in Chinese.
HUD_LABELS_ZH = {
  "eco": "节能",
  "safe": "安全",
  "norm": "标准",
...
def _hud_label(label: object) -> str:
  text = str(label)
  return HUD_LABELS_ZH.get(text, text)
...
class HudRenderer(Widget):
  def __init__(self):
    ...
    self._date_text = ""
    self._record_button_rect = rl.Rectangle(0, 0, 0, 0)
    self._bsm_lane_phase = 0.0
    self._bsm_lane_last_time = time.monotonic()
    self._record_button_event_frame = -1
  ...
  def _render(self, rect: rl.Rectangle) -> None:
    ...
    # Keep the optional obstacle diagnostic isolated from the main HUD.  A
    # malformed/stale radar message must never terminate the UI process.
    try:
      self._draw_obstacle_hud(rect)
    except Exception:
      pass
    try:
      self._draw_bsm_side_icon(rect)
    except Exception:
      # BSM is optional HUD decoration; never prevent the main onroad view
      # from starting if a device has an incomplete radar message/API.
      pass
  ...
  def _draw_bsm_side_icon(self, rect: rl.Rectangle) -> None:
    sm = ui_state.sm
    try: cs, rs = sm['carState'], sm['radarState']
    except Exception: return
    def nearest(group, single, blind):
      try: vals = list(getattr(rs, group))
      except Exception:
        try: vals = [getattr(rs, single)]
        except Exception: vals = []
      ds = []
      for t in vals:
        try:
          if bool(getattr(t, 'status', False)):
            d = float(getattr(t, 'dRel', 0.0))
            if -40.0 < d < 80.0 and abs(d) > 0.2: ds.append(d)
        except Exception: pass
      if ds:
        d = min(ds, key=abs)
        # Some BSM implementations expose only a positive range while the
        # blind-spot flag itself denotes a vehicle behind the ego car.
        if blind and d > 0.0:
          d = -d
        return True, d
      return bool(blind), 0.0
    lo, ld = nearest('leadsLeft', 'leadLeft', getattr(cs, 'leftBlindspot', False)); ro, rd = nearest('leadsRight', 'leadRight', getattr(cs, 'rightBlindspot', False))
    # Place the BSM card between the time card and device-status cards. Match
    # the 475 px width of the device-status/main HUD card.
    panel_w, panel_h = 475, 300
    panel_x = int(rect.x + 20)
    # Stretch between the lower edge of the time card and the upper edge of
    # the device-status row.
    panel_y = int(rect.y + UI_CONFIG.header_height - 90)
    cx, cy = panel_x + panel_w // 2, panel_y + panel_h // 2
    self._draw_round_box(panel_x, panel_y, panel_w, panel_h,
                         rl.Color(0, 0, 0, 72), line_color=rl.WHITE,
                         roundness=30.0 / panel_h, segments=12,
                         line_thickness=2)
    # Dashed lane boundaries are best-effort UI decoration. Keep rendering
    # guarded so a device-specific pyray build cannot take down the HUD.
    try:
      lane_color = rl.Color(255, 255, 255, 220)
      lane_x_left, lane_x_right = cx - 72, cx + 72
      dash_h, gap_h = 16, 12
      y_end = panel_y + panel_h - 10
      # Advance by elapsed real time so animation is visible even when the
      # HUD frame rate varies. Read speed from both common carState layouts.
      state_out = getattr(cs, "out", None)
      speed_ms = getattr(state_out, "vEgo", None) if state_out is not None else None
      if speed_ms is None:
        speed_ms = getattr(cs, "vEgo", 0.0)
      try:
        speed_kph = max(0.0, float(speed_ms) * CV.MS_TO_KPH)
      except (TypeError, ValueError):
        speed_kph = 0.0
      now = time.monotonic()
      dt = min(0.10, max(0.0, now - self._bsm_lane_last_time))
      self._bsm_lane_last_time = now
      # No movement when stopped. Vehicle speed alone controls the lane
      # marking scroll rate, so a stationary car has a completely static card.
      phase_rate = min(220.0, speed_kph * 4.0) if speed_kph > 0.5 else 0.0
      self._bsm_lane_phase = (self._bsm_lane_phase + phase_rate * dt) % (dash_h + gap_h)
      y = panel_y + 10 + self._bsm_lane_phase - (dash_h + gap_h)
      while y < y_end:
        dash_start = max(y, panel_y + 10)
        dash_end = min(y + dash_h, y_end)
        if dash_end > dash_start:
          dash_len = max(1, int(dash_end - dash_start))
          rl.draw_rectangle(int(lane_x_left) - 1, int(dash_start), 3, dash_len, lane_color)
          rl.draw_rectangle(int(lane_x_right) - 1, int(dash_start), 3, dash_len, lane_color)
        y += dash_h + gap_h
    except Exception:
      pass
    # Ego vehicle is kept at the upper part of the card; side vehicles are
    # rendered below it to match the requested top-down layout.
    ego = rl.Rectangle(cx-22, panel_y+28, 44, 84); rl.draw_rectangle_rounded(ego,.22,6,rl.Color(80,190,255,225)); rl.draw_rectangle_rounded_lines_ex(ego,.22,6,2,rl.WHITE)
    def side(on,d,sign):
      if not on:return
      # BSM targets occupy the lower half of the card (behind the ego icon).
      y=int(panel_y + 225 + max(-1.0,min(1.0,d/35.0))*45); x=int(cx+sign*130); col=rl.Color(255,70,70,235) if abs(d)<6 else rl.Color(255,190,40,230); car=rl.Rectangle(x-16,y-26,32,52); rl.draw_rectangle_rounded(car,.2,5,col); rl.draw_rectangle_rounded_lines_ex(car,.2,5,2,rl.WHITE)
      draw_text_ui_style(f'{abs(d):.0f}m',x,y+30,22,rl.WHITE,font=self._font_semi_bold,border_width=1.0,shadow_offset=2.0,align='center_top',y_offset=0.0)
    side(lo,ld,-1); side(ro,rd,1)
  ...
  def _get_driving_mode_text_and_color(self) -> tuple[str, rl.Color]:
    ...
    if mode_val == 1:   # eco
      return _hud_label(tr("eco")), COLORS.GREEN_200
    if mode_val == 2:   # safe
      return _hud_label(tr("safe")), COLORS.ORANGE_200
    if mode_val == 3:   # normal
      return _hud_label(tr("norm")), COLORS.WHITE_TRANSLUCENT
    if mode_val == 4:   # high
      return _hud_label(tr("high")), COLORS.RED_200
  ...
  def _draw_carrot_lower_status(self, bx: int, by: int):
    ...
    if mode_text:
      ...
      if self._gps_has_fix():
        draw_text_ui_style(
          _hud_label("GPS"), dx, dy - 45, 30, rl.GREEN,
          font=self._font_display,
          border_width=2.0,
          shadow_offset=4.0,
          align="center_bottom",
    ...
    if active_carrot >= 2:
      ...
      draw_text_ui_style(
        _hud_label("APN"), dx, dy, 40, rl.WHITE,
        font=self._font_display,
        border_width=2.0,
        shadow_offset=4.0,
        align="center_bottom",
      ...
      draw_text_ui_style(
        _hud_label("APM"), dx, dy, 40, rl.WHITE,
        font=self._font_display,
        border_width=2.0,
        shadow_offset=4.0,
        align="center_bottom",
    ...
    if self._get_nav_path_vertex_count() > 1:
      draw_text_ui_style(
        _hud_label("ROUTE"), dx, dy - 45, 30, rl.WHITE,
        font=self._font_display,
        border_width=2.0,
        shadow_offset=4.0,
        align="center_bottom",
  ...
  def _draw_carrot_device_state(self, bx: int, by: int):
    ...
    draw_text_ui_style(_hud_label("CPU"), dx, dy - 5, 25, rl.WHITE, font=self._font_display, border_width=1.0, shadow_offset=4.0, align="center_bottom")
    ...
    draw_text_ui_style(_hud_label("MEM"), dx2, dy - 5, 25, rl.WHITE, font=self._font_display, border_width=1.0, shadow_offset=4.0, align="center_bottom")
    ...
    if self._disp_timer < 32:
      draw_text_ui_style(_hud_label("DISK"), dx3, dy - 5, 25, rl.WHITE, font=self._font_display, border_width=1.0, shadow_offset=4.0, align="center_bottom")
      ...
      draw_text_ui_style(_hud_label("VOLT"), dx3, dy - 5, 25, rl.WHITE, font=self._font_display, border_width=1.0, shadow_offset=4.0, align="center_bottom")
  ...
  def _draw_date_time(self, rect: rl.Rectangle) -> None:
    ...
    # Recording toggle: directly below the time card, with the same 100 px
    # visual height as the time text.  Keep its hit box on the HUD so the
    # parent on-road view does not interpret the tap as a sidebar toggle.
    button_w, button_h = 180, 100
    # The date line (when enabled) ends around y+190; place the button below
    # the complete time card rather than over the date text.
    # Keep a generous touch target around the visual button; C3X touch
    # coordinates can be scaled slightly differently from the render surface.
    # Place the recorder directly below the obstacle card and align right
    # edges. Keep the obstacle card geometry in sync with its renderer.
    obstacle = self._get_obstacle_hud_data()
    obstacle_title = f"障碍物  动态 {len(obstacle['moving'])}  静态 {len(obstacle['static'])}"
    obstacle_w = min(620, max(430, int(measure_text_cached(self._font_bold, obstacle_title, 42).x + 44)))
    obstacle_h = 178 if obstacle["narrow_risk"] else 132
    obstacle_right = rect.x + rect.width - 36
    record_top = rect.y + UI_CONFIG.header_height + 18 + obstacle_h + 12
    record_left = obstacle_right - (button_w + 36)
    self._record_button_rect = rl.Rectangle(record_left, record_top,
                                             button_w + 36, button_h + 24)
    recording = gui_app.is_recording()
    # Transparent by default; recording state is shown by a red translucent
    # outline/fill so the camera image remains visible underneath.
    fill = rl.Color(190, 35, 35, 70) if recording else rl.Color(0, 0, 0, 0)
    border = rl.Color(255, 45, 45, 255) if recording else rl.Color(255, 255, 255, 210)
    self._draw_round_box(int(self._record_button_rect.x), int(self._record_button_rect.y),
                         int(self._record_button_rect.width), int(self._record_button_rect.height), fill, line_color=border,
                         roundness=0.16, segments=10, line_thickness=3)
    label = "停止" if recording else "录  像"
    draw_text_ui_style(label,
                       int(self._record_button_rect.x + self._record_button_rect.width / 2),
                       int(self._record_button_rect.y + self._record_button_rect.height / 2 + 19), 42,
                       rl.WHITE, font=self._font_bold, border_width=1.5,
                       shadow_offset=3.0, align="center_bottom")
  ...
  def _refresh_date_time_text(self, now: time.struct_time) -> None:
    ...
    weekday = WEEKDAYS_ZH[(now.tm_wday + 1) % 7]
  ...
  def _handle_mouse_press(self, mouse_pos) -> None:
    if self._record_button_rect.width > 0 and rl.check_collision_point_rec(mouse_pos, self._record_button_rect):
  ...
  def _handle_mouse_event(self, mouse_event) -> None:
    """Handle the HUD recorder on press, independent of parent click routing.

    C3X builds can deliver a touch release after the camera widget has already
    consumed the press.  Toggling on the confirmed press makes the control
    reliable while the frame guard prevents duplicate toggles when both paths
    report the same event.
    """
    # Toggle is handled by _handle_mouse_press; this hook is intentionally
    # empty to avoid double-triggering on platforms that emit both callbacks.
    return
  ...
  def _get_turn_info_hud_data(self) -> dict:
    ...
    return {
      ...
      "atc_type": self._translate_nav_text(atc_type),
      "sdi_descr": sdi_descr,
      "road_name": road_name,
  ...
  def _format_eta_text(self, remain_sec: int) -> str:
    ...
    return f"到达: {remain_min:.1f}分钟({eta_tm.tm_hour:02d}:{eta_tm.tm_min:02d})"

  def _translate_nav_text(self, text: str) -> str:
    replacements = {
      "falling rock caution area": "落石注意区域", "traffic light": "红绿灯",
      "speed camera": "测速摄像头", "keep left": "靠左行驶", "keep right": "靠右行驶",
      "turn left": "左转", "turn right": "右转", "destination": "目的地",
      "roundabout": "环岛", "u-turn": "掉头",
    }
    result = str(text)
  ...
  def _draw_turn_icon(self, turn_info: int, bx: int, by: int, icon_size: int = 140):
    if turn_info == 1:
      ...
      draw_text_ui_style(
        _hud_label("TG"), bx, by + 13, 35, rl.WHITE,
        font=self._font_display,
        border_width=2.0,
        shadow_offset=4.0,
        align="center_bottom",
      ...
      draw_text_ui_style(
        f"减速:{turn_info}", bx, by + 13, 35, rl.WHITE,
        font=self._font_display,
        border_width=2.0,
        shadow_offset=4.0,
        align="center_bottom",
  ...
  def _draw_turn_info_hud(self, rect: rl.Rectangle):
    ...
    info = dict(info)
    for key in ("sdi_descr", "road_name", "tbt_main_text"):
      info[key] = self._translate_nav_text(info[key])
    ...
    if info["tbt_main_text"]:
      self._draw_text_left_bottom(
        info["tbt_main_text"], tbt_x + 20, tbt_y - 45, 40, rl.WHITE,
    ...
    if info["sdi_descr"]:
      ...
      self._draw_text_left_bottom(
        info["road_name"], tbt_x + 200, tbt_y + 177, 40, rl.WHITE,
        font=self._font_bold, border_width=1.5, shadow_offset=3.0,
      )
    ...
    if eta_text:
      self._draw_text_left_bottom(
        eta_text, tbt_x + 190, tbt_y + 50, 50, rl.WHITE,
        font=self._font_bold, border_width=2.0, shadow_offset=4.0,
      )
    ...
    if go_dist_text:
      self._draw_text_left_bottom(
        go_dist_text, tbt_x + 310, tbt_y + 100, 50, rl.WHITE,
        font=self._font_bold, border_width=2.0, shadow_offset=4.0,
      )

  def _get_obstacle_hud_data(self) -> dict:
    """Build a conservative obstacle summary from the fused radar state.

    RadarState intentionally has no native object-type field.  Until a real
    Z6 capture maps the vendor Type/Direction enumerations, labels are based
    only on measured kinematics: moving target, near-zero-speed/static target,
    and a closing side target that may be oncoming on a narrow road.
    """
    empty = {
      "moving": [],
      "static": [],
    ...
    candidates = []
    groups = ("leadOne", "leadTwo", "leadLeft", "leadRight",
              "leadsCenter", "leadsLeft", "leadsRight", "leadsCutIn")
    seen_ids = set()
    for group in groups:
      try:
        values = [getattr(radar, group)] if group.startswith("lead") and group not in ("leadsCenter", "leadsLeft", "leadsRight", "leadsCutIn") else list(getattr(radar, group))
      except Exception:
        continue
      for lead in values:
        try:
          ...
          continue
        if not (0.2 < d_rel < 180.0) or not all(math.isfinite(v) for v in (y_rel, v_rel, v_lead)):
          continue
        if track_id >= 0 and track_id in seen_ids:
          continue
        if track_id >= 0:
          seen_ids.add(track_id)
        candidates.append((d_rel, y_rel, v_rel, v_lead))

    if not candidates:
      return empty

    # A stationary object has vLead≈0 even while our own vehicle is moving;
    # a moving target has a positive absolute lead speed.  Keep the threshold
    # deliberately wide to avoid classifying quantized radar returns as static.
    static = [c for c in candidates if c[3] <= 1.5]
    moving = [c for c in candidates if c[3] > 1.5]
    try:
      ego_speed = max(0.0, float(ui_state.sm["carState"].vEgo))
    except Exception:
      ego_speed = 0.0
    oncoming = [c for c in candidates
                if c[2] < -max(3.0, 1.25 * ego_speed)
                and 0.8 < abs(c[1]) < 5.5]

    # Narrow-road risk: a closing side target inside 45 m, or both sides
    # occupied close to the ego path.  This is a warning/deceleration cue only;
    # it never synthesizes a lateral steering command.
    close_side = [c for c in candidates if c[0] < 45.0 and abs(c[1]) > 1.0]
    sides = {"left" if c[1] < 0.0 else "right" for c in close_side}
    narrow_risk = bool(oncoming) or len(sides) == 2 and min(c[0] for c in close_side) < 30.0
    return {
      "moving": sorted(moving, key=lambda c: c[0]),
      "static": sorted(static, key=lambda c: c[0]),
      "oncoming": sorted(oncoming, key=lambda c: c[0]),
      "narrow_risk": narrow_risk,
    }

  def _draw_obstacle_hud(self, rect: rl.Rectangle) -> None:
    data = self._get_obstacle_hud_data()
    # Always show the counters, including the zero/zero state, so the compact
    # obstacle panel does not disappear when radar has no current target.

    # Use the same 42 px type as the recording button and size the panel to
    # this title so its length stays visually balanced with the border.
    title = f"障碍物  动态 {len(data['moving'])}  静态 {len(data['static'])}"
    title_px = measure_text_cached(self._font_bold, title, 42).x
    # Keep enough width for the Chinese counters and clamp the panel fully
    # inside the camera viewport on the narrow C3X screen.
    panel_w = min(620, max(430, int(title_px + 44)))
    panel_h = 178 if data["narrow_risk"] else 132
    panel_x = int(max(rect.x + 12, rect.x + rect.width - panel_w - 36))
    panel_y = int(rect.y + UI_CONFIG.header_height + 18)
    border = COLORS.RED_210 if data["narrow_risk"] else COLORS.WHITE_TRANSLUCENT
    fill = rl.Color(30, 0, 0, 45) if data["narrow_risk"] else rl.Color(0, 0, 0, 0)
    self._draw_round_box(panel_x, panel_y, panel_w, panel_h, fill,
                         line_color=border, roundness=0.12, segments=12, line_thickness=3)

    draw_text_ui_style(title, panel_x + 20, panel_y + 45, 42, rl.WHITE,
                       font=self._font_bold, border_width=1.5, shadow_offset=3.0,
                       align="left_bottom")

    details = []
    if data["moving"]:
      details.append(f"行驶 {data['moving'][0][0]:.0f}米")
    if data["static"]:
      details.append(f"静止 {data['static'][0][0]:.0f}米")
    if data["oncoming"]:
      details.append(f"对向 {data['oncoming'][0][0]:.0f}米")
    detail_text = "   ".join(details)
    if detail_text:
      draw_text_ui_style(detail_text, panel_x + 22, panel_y + 94, 42, rl.YELLOW_210,
                         font=self._font_display, border_width=1.0, shadow_offset=2.0,
                         align="left_bottom")

    if data["narrow_risk"]:
      draw_text_ui_style("窄路风险  请接管并减速", panel_x + 22, panel_y + 145, 42, COLORS.RED_210,
                         font=self._font_bold, border_width=1.0, shadow_offset=2.0,
                         align="left_bottom")
