...
# Keep compact Mici HUD labels in Chinese, including MEM/DISK usage labels.
HUD_LABELS_ZH = {
  "eco": "节能", "safe": "安全", "norm": "标准", "high": "高效",
  "vturn": "弯道", "cam": "测速", "section": "区间测速", "bump": "减速带",
  "police": "警示", "waze": "导航", "road": "道路", "turn": "转向",
  "route": "路线", "curve": "弯道", "school": "学校", "gas": "加油站",
  "model": "模型", "apply": "减速", "MAX": "最大", "GPS": "定位",
  "APN": "导航", "APM": "手动", "ROUTE": "路线", "LIMIT": "限速",
  "CAM": "测速", "NAVI": "导航", "vNAVI": "车载导航", "CPU": "处理器",
  "MEM": "运存占用", "DISK": "存储占用", "VOLT": "电压", "TG": "引导",
  "brake hold": "刹车保持", "soft hold": "柔和保持", "carrot": "辅助驾驶",
}


def _hud_label(label: object) -> str:
  return HUD_LABELS_ZH.get(str(label), str(label))
...
class HudRenderer(Widget):
  ...
  def _get_obstacle_hud_data(self) -> dict:
    empty = {"moving": [], "static": [], "oncoming": [], "narrow_risk": False}
    try:
      sm = ui_state.sm
    ...
    for group in groups:
      try:
        value = getattr(radar, group)
        values = [value] if group.startswith("lead") and group not in ("leadsCenter", "leadsLeft", "leadsRight", "leadsCutIn") else list(value)
      except Exception:
        continue
      for lead in values:
        try:
          if not bool(lead.status):
            continue
          d_rel, y_rel, v_rel, v_lead = map(float, (lead.dRel, lead.yRel, lead.vRel, lead.vLead))
    ...
    if not candidates:
      return empty
    static = sorted((c for c in candidates if c[3] <= 1.5), key=lambda c: c[0])
    moving = sorted((c for c in candidates if c[3] > 1.5), key=lambda c: c[0])
    oncoming = sorted((c for c in candidates if c[2] < -3.0 and 0.8 < abs(c[1]) < 5.5), key=lambda c: c[0])
    close_side = [c for c in candidates if c[0] < 45.0 and abs(c[1]) > 1.0]
    sides = {"left" if c[1] < 0.0 else "right" for c in close_side}
    return {"moving": moving, "static": static, "oncoming": oncoming,
            "narrow_risk": bool(oncoming) or (len(sides) == 2 and close_side and min(c[0] for c in close_side) < 30.0)}

  def _draw_obstacle_hud(self, rect: rl.Rectangle) -> None:
    data = self._get_obstacle_hud_data()
    if not data["moving"] and not data["static"] and not data["oncoming"]:
      return
    panel_w, panel_h = 360, 88 if not data["narrow_risk"] else 112
    panel_x = int(rect.x + rect.width - panel_w - 20)
    panel_y = int(rect.y + 190)
    border = rl.Color(255, 80, 60, 230) if data["narrow_risk"] else rl.Color(255, 255, 255, 170)
    fill = rl.Color(30, 0, 0, 190) if data["narrow_risk"] else rl.Color(0, 0, 0, 155)
    rl.draw_rectangle_rounded(rl.Rectangle(panel_x, panel_y, panel_w, panel_h), 0.12, 10, fill)
    rl.draw_rectangle_rounded_lines_ex(rl.Rectangle(panel_x, panel_y, panel_w, panel_h), 0.12, 10, 2, border)
    title = f"动态 {len(data['moving'])}   静止 {len(data['static'])}"
    draw_text_ui_style(title, panel_x + 14, panel_y + 30, 25, rl.WHITE, font=self._font_bold, border_width=1.0, shadow_offset=2.0, align="left_bottom")
    details = []
    if data["moving"]:
      details.append(f"行驶 {data['moving'][0][0]:.0f}米")
    if data["static"]:
      details.append(f"障碍 {data['static'][0][0]:.0f}米")
    if data["oncoming"]:
      details.append(f"对向 {data['oncoming'][0][0]:.0f}米")
    if details:
      draw_text_ui_style("   ".join(details), panel_x + 14, panel_y + 61, 22, rl.YELLOW, font=self._font_display, border_width=1.0, shadow_offset=2.0, align="left_bottom")
    if data["narrow_risk"]:
      draw_text_ui_style("窄路风险  请接管并减速", panel_x + 14, panel_y + 94, 22, border, font=self._font_bold, border_width=1.0, shadow_offset=2.0, align="left_bottom")
  ...
  def _draw_wheel_side_info(self, wheel_txt, pos_x: int, pos_y: int) -> None:
    ...
    if show_date_time != 0:
      ...
      weekday = weekdays_zh[(now.weekday() + 1) % 7]
    ...
    if True: #show_set or self._debug_speed_panel:
      ...
      if ov.active:
        ...
        if not ui_state.is_metric:
          set_speed *= KM_TO_MILE
        set_text = str(int(round(set_speed)))
        set_label_text = _hud_label(ov.label)
        ...
        if self._debug_speed_panel:
          set_text = str(111)
          set_color = rl.Color(255, 165, 0, 230)
          set_label_text = _hud_label("vturn")
