...
class RadarD:
  ...
  def compute_leads(self, v_ego, tracks, md, lead_prob, front_tracks: dict[int, Track] | None = None):
    ...
    external_groups = (
      ...
      # Z6's static-obstacle bank has no camera lead and therefore normally
      # has vLead≈0.  Promote a continuously confirmed in-lane static return
      # to leadTwo so MPC brakes for it instead of merely drawing a point.
      [
        ld for ld in center_list
        if external_candidate(ld) and ld['vLead'] <= 2.0
        # At low speed, water barriers/cones can first appear very close to
        # the bumper and with a larger lateral residual than a full-size car.
        # Accept the shorter range and wider path band only below 60 km/h;
        # retain the original stricter gate at higher speed to avoid roadside
        # clutter becoming a braking target.
        and (
          (self.v_ego <= 16.7 and ld['dRel'] > 0.8 and abs(ld.get('dPath', 99.0)) < 1.8)
          or
          (self.v_ego > 16.7 and ld['dRel'] > 2.0 and abs(ld.get('dPath', 99.0)) < 1.25)
        )
      ],
      # A closing object just outside the lane is a common narrow-road
      # oncoming-car signature.  Use a bounded longitudinal precaution only;
      # lateral avoidance remains driver-supervised and is never synthesized
      # from an uncalibrated native Direction value.
      [
        ld for ld in (
          c.get_RadarState(0.0, 0.0)
          for c in tracks.values()
          if self.car_brand == "changan" and c.measured and c.cnt > 5
        )
        if external_candidate(ld) and 2.0 < ld['dRel'] < 60.0
        # A stationary roadside object approaches at roughly -v_ego.  Require
        # a stronger-than-static closure before treating a side return as a
        # possible oncoming vehicle on a narrow road.
        and ld['vRel'] < -max(3.0, 1.25 * self.v_ego)
        and 1.0 < abs(ld['yRel']) < 5.0
      ],
    )
