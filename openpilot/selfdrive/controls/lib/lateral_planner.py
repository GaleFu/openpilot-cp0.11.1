...
class LateralPlanner:
  def __init__(self, CP, debug=False):
    ...
    self.curve_speed = 0
    # Timed memory for sharp bends when the camera temporarily loses the
    # lane boundary (e.g. an unmarked right-angle turn).
    self.turn_curvature = 0.0
  ...
  def update(self, sm, carrot):
    ...
    # Preserve the last confirmed turn direction instead of immediately
    # unwinding toward a straight model path. Model yaw-rate is used as a
    # second source of curvature evidence when measured curvature is small.
    model_curvature = 0.0
    if len(self.plan_yaw_rate) > 5 and len(self.v_plan) > 5:
      model_curvature = float(self.plan_yaw_rate[5]) / max(float(self.v_plan[5]), 2.0)
    curvature_evidence = float(measured_curvature)
    if abs(model_curvature) > abs(curvature_evidence):
      curvature_evidence = model_curvature
    if abs(curvature_evidence) > 0.010:
      self.turn_hold_frames = max(1, int(1.5 / DT_MDL))
    else:
      self.turn_hold_frames = max(0, self.turn_hold_frames - 1)
    self.turn_curvature = 0.95 * self.turn_curvature + 0.05 * curvature_evidence

    if (not self.lanelines_active and self.turn_hold_frames > 0
        and abs(self.turn_curvature) > 0.008):
      x = np.maximum(self.path_xyz[:, 0], 0.0)
      arc_y = 0.5 * self.turn_curvature * x * x
      blend = np.clip((x - 1.5) / 15.0, 0.0, 1.0) * 0.72
      correction = np.clip(arc_y - self.path_xyz[:, 1], -1.25, 1.25)
      self.path_xyz[:, 1] += blend * correction
