...
class LongitudinalPlanner:
  ...
  def update(self, sm, carrot):
    ...
    # Soften the initial brake touch when the following gap is still clearly
    # safe.  The MPC remains authoritative; this only rate-limits the first
    # transition into mild deceleration and is bypassed for closing/emergency
    # situations where the gap is no longer generous.
    try:
      lead = sm['radarState'].leadOne
      # Apply this only with a generous gap; emergency closing remains under
      # the original MPC response.
      # Keep the original activation distance; only the deceleration ramp is
      # softened below, so the desired following distance is unchanged.
      gap_safe = (bool(lead.status) and float(lead.dRel) >
                  max(8.0, float(self.mpc.desired_distance) + 3.0))
      closing_mild = float(lead.vRel) > -4.0
    except Exception:
      gap_safe, closing_mild = False, False
    if gap_safe and closing_mild and not sm['carState'].brakePressed:
      # About 0.4 m/s²/s at the 20 Hz planner rate for a softer first touch.
      # 0.3 m/s²/s at the 20 Hz planner rate: very gentle initial pedal rise.
      min_allowed = self._soft_brake_prev - 0.015
      if output_a_target < self._soft_brake_prev:
        output_a_target = max(output_a_target, min_allowed)
    self._soft_brake_prev = float(output_a_target)
