...
class Controls:
  ...
  def state_control(self):
    ...
    # Lateral planner publishes OA:1 only after low-speed obstacle, lane
    # boundaries and the selected side's free space are all confirmed.
    # Force that plan through even when normal operation is laneless/raw model.
    avoid_active = 'OA:1' in str(getattr(lat_plan, 'latDebugText', ''))
    ...
    if not CC.latActive:
      ...
      if (getattr(lat_plan, 'useLaneLines', False) or avoid_active) and len(lat_plan.curvatures) > 0:
    ...
    elif self.lanefull_mode_enabled or avoid_active:
