...
void ignition_can_hook(CANPacket_t *to_push) {
  ...
  if (bus == 0) {
    ...
    // Changan/Deepal/Oshan exception. Z6 generations expose the running
    // power state on either PEPS 0x262 or the body gateway 0x28B.
    if ((addr == 0x262) && (len == 8)) {
      ignition_can = ((GET_BYTE(to_push, 2) >> 6) & 0x3U) == 2U;
      ignition_can_cnt = 0U;
    }

    if ((addr == 0x28B) && (len == 8)) {
      ignition_can = ((GET_BYTE(to_push, 4) >> 2) & 0x3U) == 2U;
      ignition_can_cnt = 0U;
    }
