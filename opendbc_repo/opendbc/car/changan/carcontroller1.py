...
Z6_SIGNAL_ENTRY_TARGET_ANGLE = 100.0
Z6_SIGNAL_ENTRY_MIN_CMD_ANGLE = 20.0
Z6_SIGNAL_ENTRY_HANDOFF_ACTUAL_ANGLE = 90.0
...
Z6_MID_TURN_HOLD_START_ANGLE = 70.0
Z6_MID_TURN_HOLD_MIN_ANGLE = 50.0
Z6_MID_TURN_HOLD_MAX_ANGLE = 140.0
Z6_MID_TURN_HOLD_FRAMES = 70           # 1.4 s at the 50 Hz CAN update
Z6_MID_TURN_HANDOFF_RAMP_FRAMES = 45   # then return to the model smoothly
...
class CarController(CarControllerBase):
  def __init__(self, dbc_names, CP):
    ...
    self.accel_last = 0.
    # Explicit throttle-to-brake handoff.  The Changan ECU can retain drive
    # torque for a short period after a positive ACC command is replaced by a
    # negative one.  Keep the longitudinal command neutral for a bounded
    # number of 50 Hz updates before allowing CDD/brake torque.
    self.z6_throttle_brake_handoff_frames = 0
  ...
  def _apply_z6_mid_turn_hold(self, desired_angle: float, actual_angle: float,
    ...
    hold_angle = max(
      ...
      min(Z6_MID_TURN_HOLD_MAX_ANGLE, abs(actual_angle) * 0.85),
    )
    ...
    if abs(self.apply_angle_last) > hold_angle:
      hold_angle = min(Z6_MID_TURN_HOLD_MAX_ANGLE,
                       abs(self.apply_angle_last) * 0.80)
  ...
  def _apply_z6_low_speed_drive_filter(self, accel_cmd: float,
                                       v_ego_kph: float,
                                       standstill: bool) -> float:
    """Low-pass only positive drive torque below 40 km/h.

    This removes the repeating positive-throttle peaks without creating a
    zero deadband. Negative requests bypass the filter so braking response is
    unchanged.
    """
    low_speed_limit = getattr(CarControllerParams, "Z6_LOW_SPEED_BRAKE_KPH", 40.0)
    if not (self.is_gas and not standstill and 0.1 < v_ego_kph <= low_speed_limit):
    ...
    alpha_up = getattr(CarControllerParams, "Z6_LOW_SPEED_DRIVE_FILTER_UP", 0.06)
    alpha_down = getattr(CarControllerParams, "Z6_LOW_SPEED_DRIVE_FILTER_DOWN", 0.10)
    alpha = alpha_up if accel_cmd > self.z6_low_speed_drive_filtered else alpha_down
    self.z6_low_speed_drive_filtered += alpha * (accel_cmd - self.z6_low_speed_drive_filtered)
  ...
  def _apply_z6_rapid_brake_smoothing(self, accel_cmd: float,
    ...
    if accel_cmd <= critical:
      ...
      down_step = getattr(CarControllerParams, "Z6_RAPID_BRAKE_DOWN_STEP", 0.16)

    if previous >= 0.0:
      # Drop drive torque immediately, then enter CDD with a bounded request.
      entry_step = getattr(CarControllerParams, "Z6_RAPID_BRAKE_ENTRY_STEP", 0.20)
      if accel_cmd <= critical:
    ...
    if self.CP.openpilotLongitudinalControl:
      ...
      if self.frame % CarControllerParams.ACC_CONTROL_STEP == 0:
        ...
        if long_active:
          ...
          # Never overlap commanded drive torque and normal braking.  A
          # positive command from the previous CAN cycle must first be
          # replaced by neutral for two cycles (about 40 ms at 50 Hz), giving
          # the ECU/actuator time to release throttle torque.  Critical
          # requests bypass this comfort handoff so emergency braking remains
          # available without delay.
          handoff_emergency = getattr(CarControllerParams,
                                      "Z6_THROTTLE_BRAKE_HANDOFF_EMERGENCY_ACCEL", -2.80)
          if self.z6_throttle_brake_handoff_frames > 0:
            if accel_cmd < 0.0 and accel_cmd > handoff_emergency:
              accel_cmd = 0.0
              self.z6_throttle_brake_handoff_frames -= 1
          elif (self.accel_last > 0.02 and accel_cmd < 0.0 and
                accel_cmd > handoff_emergency):
            self.z6_throttle_brake_handoff_frames = max(
              0, int(getattr(CarControllerParams,
                             "Z6_THROTTLE_BRAKE_HANDOFF_FRAMES", 2)) - 1)
            accel_cmd = 0.0
          low_speed_limit = getattr(CarControllerParams, "Z6_LOW_SPEED_BRAKE_KPH", 40.0)
          ...
          # Final low-speed drive-torque shaping.  The Z6 torque map is steep
          # around zero, so small PID peaks become noticeable surges below
          # 40 km/h. Compress only positive acceleration; braking is untouched.
          if (self.is_gas and not standstill and v_ego_kph <= low_speed_limit and
              accel_cmd > 0.0):
            accel_cmd = min(
              accel_cmd * getattr(CarControllerParams,
                                  "Z6_LOW_SPEED_DRIVE_SCALE", 0.65),
              getattr(CarControllerParams,
                      "Z6_LOW_SPEED_DRIVE_ACCEL_CAP", 0.20),
            )
            # Prevent activation overshoot: when actual speed is at/near the
            # driver's set speed, do not keep applying positive torque.
            try:
              set_speed_kph = float(CC.hudControl.setSpeed) * CV.MS_TO_KPH
            except Exception:
              set_speed_kph = 0.0
            if set_speed_kph > 1.0:
              margin = getattr(CarControllerParams, "Z6_LOW_SPEED_SET_SPEED_MARGIN_KPH", 0.4)
