...
class CarControllerParams:
  ...
  # Deep-turn headroom. The old 170 deg cap limited the command in U-turns
  # where the wheel genuinely needs 200-300 deg, so the car ran wide. 260 deg
  # stays below the 400 deg outward-window start and the 720 deg EPS/DBC and
  # Panda boundaries; the per-frame delta and EPS_CMD_ERROR windows still
  # shape how quickly the command walks toward it.
  STEER_COMMAND_MAX_ANGLE = 260.0
  # The Z6 EPS authorizes the external IACC angle command against an internal
  # lateral-acceleration budget. Measured rejects (~90 deg at 40 km/h, ~180 deg
  # below 30 km/h) both map to ~4.8 m/s^2 with steerRatio 14.5 and wheelbase
  # 2.795, while the OEM APA session (EPS-internal path) reaches 490 deg. This
  ...
  # torque requests up to +/-5 Nm (calibratable); 4.0 is a first incremental
  # step above the proven 3.0 to test whether ground-reaction torque at speed
  # was starving angle tracking. Must match Panda safety raw limits 1224/824.
  STEER_TORQUE_LIMIT = 4.0
  STEER_THRESHOLD = 1.0
  ...
  Z6_LOW_SPEED_ACCEL_UP_STEP = 0.07
  Z6_LOW_SPEED_ACCEL_DOWN_STEP = 0.10
  Z6_LOW_SPEED_DRIVE_FILTER_UP = 0.12
  Z6_LOW_SPEED_DRIVE_FILTER_DOWN = 0.20
  Z6_LOW_SPEED_DRIVE_SCALE = 0.80
  Z6_LOW_SPEED_DRIVE_ACCEL_CAP = 0.18
  Z6_LOW_SPEED_SET_SPEED_MARGIN_KPH = 0.5
  Z6_LOW_SPEED_SET_SPEED_ACCEL_CAP = 0.05
  ...
  Z6_LONG_ACTIVATION_ACCEL_CAP = 0.10
  ...
  Z6_RAPID_BRAKE_ENTRY_STEP = 0.08
  Z6_RAPID_BRAKE_DOWN_STEP = 0.10
